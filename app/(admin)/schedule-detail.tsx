...
export default function ScheduleDetailScreen() {
  ...
  const handleToggleAttendance = async (profileId: string) => {
    ...
    if (attendanceIds.has(profileId)) {
      await supabase.from('attendance').delete().eq('schedule_id', id).eq('profile_id', profileId)
  ...
  const doDeleteSeries = async () => {
    ...
    setDeleting(true)
    await supabase.from('schedules').delete().eq('series_id', schedule.series_id)
    setDeleting(false)
  ...
  if (!schedule) {
    return <View style={styles.center}><Text style={{ color: '#888' }}>Nie znaleziono służby</Text></View>
  }
    ...
    <>
      ...
      <ScrollView style={styles.container} contentContainerStyle={[styles.content, { paddingBottom: Math.max(insets.bottom, 16) }]}>
        ...
        <View style={[styles.infoCard, { borderLeftWidth: 4, borderLeftColor: cat.color }]}>
          ...
          {schedule.series_id && (
            <View style={[styles.infoRow, styles.seriesRow]}>
              <Ionicons name="list-outline" size={15} color="#534AB7" />
        ...
        <View style={styles.attendanceCard}>
          ...
          <View style={styles.attendanceStat}>
            <Text style={[styles.attendanceNum, { color: '#27ae60' }]}>{presentCount}</Text>
            <Text style={styles.attendanceLabel}>Obecnych</Text>
          </View>
          <View style={styles.attendanceDivider} />
          <View style={styles.attendanceStat}>
            <Text style={[styles.attendanceNum, { color: '#e67e22' }]}>{totalCount - presentCount}</Text>
        ...
        <View style={styles.sectionHeader}>
          ...
          <TouchableOpacity style={styles.addMemberBtn} onPress={openAddModal}>
            <Ionicons name="person-add-outline" size={15} color="#534AB7" />
              ...
              <View key={a.id} style={styles.assignmentCard}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    {(a.status === 'excused' || a.status === 'confirmed') && a.absence_reason && (
                      <View style={styles.absenceRow}>
                        <Ionicons name="chatbubble-outline" size={11} color="#e74c3c" />
                ...
                <View style={styles.assigneeRight}>
                  <TouchableOpacity onPress={() => setStatusModalAssignment(a)} disabled={updatingId === a.id}>
                    <View style={[styles.statusPill, { backgroundColor: (STATUS_COLORS[a.status] ?? '#888') + '22' }]}>
                      <Text style={[styles.statusText, { color: STATUS_COLORS[a.status] ?? '#888' }]}>
                    ...
                    {togglingAttendance === a.profile_id ? (
                      ...
                      <TouchableOpacity onPress={() => handleToggleAttendance(a.profile_id)} hitSlop={8}>
                        <Ionicons
                          ...
                          size={24}
                          color={attendanceIds.has(a.profile_id) ? '#27ae60' : '#ccc'}
                        />
                      </TouchableOpacity>
                    )}
                    {updatingId === a.id ? (
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalSheet}>
              ...
              <View style={styles.searchBox}>
                ...
                <TextInput
                  style={styles.searchInput} placeholder="Szukaj po imieniu..." placeholderTextColor="#aaa"
                  ...
                  filteredMembers.map(m => (
                    <TouchableOpacity key={m.id} style={styles.memberRow} onPress={() => handleAdd(m)} disabled={adding}>
                      <View style={styles.memberAvatar}>
                        <Ionicons name="person" size={15} color="#534AB7" />
                      </View>
                      <Text style={styles.memberName}>{m.full_name}</Text>
                      {adding ? <ActivityIndicator size="small" color="#534AB7" /> : <Ionicons name="add-circle-outline" size={20} color="#534AB7" />}
                    </TouchableOpacity>
                  ))
                )}
              </ScrollView>
...
const styles = StyleSheet.create({
  ...
  seriesRow: { paddingTop: 6, marginTop: 4, borderTopWidth: 1, borderTopColor: '#f0f0f0' },
  seriesText: { flex: 1, fontSize: 13, color: '#534AB7' },
  ...
  attendanceNum: { fontSize: 26, fontWeight: '800', color: '#1a1a1a' },
  attendanceLabel: { fontSize: 11, color: '#888' },
  attendanceDivider: { width: 1, height: 36, backgroundColor: '#f0f0f0' },
  ...
  addMemberBtn: {
    ...
    backgroundColor: '#534AB711', borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6,
    borderWidth: 1, borderColor: '#534AB733',
  },
  addMemberText: { fontSize: 13, fontWeight: '600', color: '#534AB7' },

  empty: { alignItems: 'center', gap: 8, paddingVertical: 32 },
  emptyTitle: { fontSize: 15, fontWeight: '600', color: '#aaa' },
  emptySubtitle: { fontSize: 13, color: '#ccc', textAlign: 'center' },
  ...
  avatar: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#534AB711', justifyContent: 'center', alignItems: 'center' },
  assigneeName: { fontSize: 14, fontWeight: '600', color: '#1a1a1a' },
  assigneePhone: { fontSize: 12, color: '#888', marginTop: 1 },
  absenceRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 4, marginTop: 3 },
  absenceText: { fontSize: 11, color: '#e74c3c', fontStyle: 'italic', flex: 1 },
  ...
  modalSheet: { backgroundColor: '#fff', borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20, maxHeight: '75%' },
  ...
  statusOptionText: { flex: 1, fontSize: 15, color: '#1a1a1a' },
  statusDot: { width: 10, height: 10, borderRadius: 5 },
  searchBox: {
    flexDirection: 'row', alignItems: 'center', gap: 8,
    backgroundColor: '#f5f5f5', borderRadius: 10, paddingHorizontal: 10, paddingVertical: 9, marginBottom: 10,
  },
  ...
  memberAvatar: { width: 32, height: 32, borderRadius: 16, backgroundColor: '#534AB711', justifyContent: 'center', alignItems: 'center' },
  ...
  deleteSheetTitle: { fontSize: 17, fontWeight: '700', color: '#1a1a1a', marginBottom: 2 },
  deleteSheetSub: { fontSize: 13, color: '#888', marginBottom: 16 },
  deleteOption: {
    flexDirection: 'row', alignItems: 'center', gap: 14,
    backgroundColor: '#fff5f5', borderRadius: 14, padding: 14, marginBottom: 8,
  },
  deleteOptionSeriesCard: { backgroundColor: '#fdf0f0' },
  deleteOptionIcon: {
    width: 40, height: 40, borderRadius: 12,
    backgroundColor: '#e74c3c18', justifyContent: 'center', alignItems: 'center',
  },
  deleteOptionTitle: { fontSize: 15, fontWeight: '600', color: '#e74c3c' },
  deleteOptionSub: { fontSize: 12, color: '#aaa', marginTop: 2 },
  deleteCancelBtn: {
    marginTop: 4, paddingVertical: 14, borderRadius: 14,
    backgroundColor: '#f5f5f5', alignItems: 'center',
  },
  deleteCancelText: { fontSize: 15, fontWeight: '600', color: '#555' },
})
