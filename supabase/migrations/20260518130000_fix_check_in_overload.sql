-- Drop both overloaded versions to resolve ambiguity
DROP FUNCTION IF EXISTS check_in_and_award_points(uuid, uuid, uuid);
DROP FUNCTION IF EXISTS check_in_and_award_points(uuid, uuid, uuid, text, double precision, double precision);

-- Single unified function with optional method/location params
...
BEGIN
  ...
  IF v_schedule_category = 'msza' THEN
    v_service_type := CASE WHEN v_assignment_id IS NOT NULL THEN 'msza_assigned' ELSE 'msza_extra' END;
